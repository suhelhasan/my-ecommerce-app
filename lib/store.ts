...
//   if (idx >= 0) existing.items[idx].qty += qty;
//   else existing.items.push({ itemId, qty });
...
// export function getCart(userId: ID) {
//   const c = CARTS.get(userId) ?? { userId, items: [], updatedAt: Date.now() };
//   return c;
// }
